...
if [ "$#" -lt 3 ]; then
    die "Atleast 3 arguments required, $# provided. 1st: Action, 2nd: Group, 3rd: Env, 4th: Profile, 5th: KMSKeyID"
else
...
'deploy')
    ...
    # Construct file path
    # ssm_config_path="config/$2/$3/ssm.config.json"
    # ssm_key_prefix="/$2/$3/${config_app^^}/"
    # config_ssm_parameters=""
    # for row in $(cat ${ssm_config_path} | jq -c '.parameters | to_entries | .[] '); do
    #     KEY=$(echo ${row} | jq -r '.key')
    #     if [ -z "$4" ]; then
    #         VALUE=$(aws ssm get-parameter --with-decryption --name "${ssm_key_prefix}${KEY}" --query Parameter.Value)
    #     else
    #         VALUE=$(aws ssm get-parameter --with-decryption --name "${ssm_key_prefix}${KEY}" --query Parameter.Value --profile "$4")
    #     fi
    ...
    if [ -z "$5" ]; then
        ...
        kms_parameters="KMSKeyID=${5}"
        script_parameters="${script_parameters} ${kms_parameters}"
    fi

    #combined params to be passed to template
    # deploy_parameters="${config_parameters} ${script_parameters} ${config_ssm_parameters}"
